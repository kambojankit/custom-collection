...
public class CustomMinHeap<T extends Comparable<T>> implements CustomHeap<T> {
    ...
    public CustomMinHeap(Class<T> clazz) {
    ...
    private void heapifyUp(int index) {
        ...
        if(isParentGreater(index, current)) {
    ...
    private boolean isParentGreater(int currentIndex, T current) {
    ...
    private void heapifyDown(int index) {
        ...
        if(hasRightChild(index)) {
            childIndex = hasLeftChild(index) && isLeftChildSmallerThanRight(index) ? getLeftChildIndex(index) : getRightChildIndex(index);
        }

        if(isCurrentGreaterThanChild(current, childIndex)) {
            swap(index, childIndex);
            heapifyDown(childIndex);
        }
    }

    private boolean isCurrentGreaterThanChild(T current, int childIndex) {
    ...
    private boolean isLeftChildSmallerThanRight(int index) {
